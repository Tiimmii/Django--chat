let url = `ws://${window.location.host}/chat/socket-server/`

const USER_ID = document.getElementById("logged-in-user").value
console.log(document.getElementById("logged-in-user").value)

let input_message = $('#input-message')
const currentDate = new Date();
const options = { weekday:'long', year:'numeric', time: 'numeric'};
...
chatSocket.onmessage = async function(e){
    ...
    newMessage(type, message, sent_by_id, thread_id)
    // if(data.type === 'chat'){
    //     if(data.sent_by == USER_ID){
            // message_element = `<div style="text-align: right;">
            // <p>${data.message}</p>
            // <span>${date}<span>
            // </div>`
    //     }
    //     else{
            // message_element = `<div style="text-align: left;">
            // <p>${data.message}</p>
            // <span>${date}<span>
            // </div>` 
    //     }
...
function newMessage(type, message, sent_by_id, thread_id){
    if ($.trim(message) === '') {
		return false;
	}
    let message_element;
    ...
    let contact = $('.contact-li[chat-id="'+chat_id+'"]')
    if(type == 'chat'){
        if(sent_by_id==USER_ID){
            message_element = `<div class="d-flex mb-4 replied">
            ...
            message_element = `<div class="d-flex mb-4 received">
            <div class="img_cont_msg">
...
form.on('submit', async function(e){
    ...
    chatSocket.send(JSON.stringify({
        'message':message,
        'sent_by': USER_ID,
        'sent_to': get_other_user_id(),
        'thread_id': get_thread_id(), 
    }))
   }
})
let contacts = $('.contact-li')
    contacts.on('click', function(){
        $('.contacts .active').removeClass('active')
        $(this).addClass('active') 
    
        let thread_id = $(this).attr('chat-id');
        $('.messages-wrapper.is_active').removeClass('is_active');
        $('.messages-wrapper[chat-id="' + thread_id +'"]').addClass('is_active');
    
    })
function get_thread_id(){
    let chat_id = $('.contact-li.active').attr('chat-id')
